...
export const createSurvey = async (req: Request, res: ExpressResponse) => {
  try {
    ...
    const survey: Survey = {
      ...
      questions: typeof surveyRow.questions === 'string' ? JSON.parse(surveyRow.questions) : surveyRow.questions,
      categories: typeof surveyRow.categories === 'string' ? JSON.parse(surveyRow.categories) : surveyRow.categories,
